<div>
    <!-- Modal -->
    <div wire:ignore.self class="modal fade" id="modalFormatStudent" tabindex="-1"
        aria-labelledby="modalFormatStudentLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalFormatStudentLabel">Crear Mi Formato</h5>
                    <button id="btn-add-format-title" type="button" class="close" data-dismiss="modal"
                        aria-label="Close">
                ...
                <div class="modal-body">
                    ...
                    @if (!$this->xformat_id)
                        <button onclick="saveNewFormatStudentJS()" type="button" class="btn btn-orange btn-sm mb-4"><i
                                class="fa fa-check mr-1"></i>Crear Formato</button>
                    @endif
                    @if ($this->xformat_id)
                        <button onclick="addTitlePartNewJS()" type="button" class="btn btn-success btn-sm mb-4"><i
                                ...
                                <li>
                                    @if ($part['id'])
                                        <div class="btn-group mr-3">
                                            <button
                                                onclick="addSubPartFormatNewJS({{ $k }},{{ $part['id'] }})"
                                                type="button" class="btn btn-secondary btn-sm">
                                                <i class="fa fa-plus"></i>
                                            </button>
                                            <button onclick="deletePartStudentNewJS({{ $part['id'] }})"
                                                type="button" class="btn btn-secondary btn-sm">
                                                <i class="fa fa-trash-alt"></i>
                                            </button>
                                        </div>
                                    @endif
                                    <a class="formattitlereload" href="#" id="formattitle{{ $k }}"
                                        data-type="text" data-pk="{{ $part['id'] }}"
                                        data-title="Escriba Titulo">{{ $part['description'] }}</a>
    ...
    <script>
        ...
        function addSubPartFormatNewJS(k, id) {
            ...
            newLi.innerHTML = '<a class="formattitlereload" href="#" id="subformattitleNew' + k + id +
                '" data-type="text" data-pk="' + id + '"></a>';
